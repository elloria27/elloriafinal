...
`;
      ...
      console.log('Sending request to update files...');
      const response = await fetch('/lovable/api/save', {
        method: 'POST',
        headers: {
      ...
      console.log('File update response status:', response.status);

      if (!response.ok) {
        const errorText = await response.text();
        console.error('File update failed:', errorText);
        throw new Error(`Failed to update configuration files: ${errorText}`);
      }

      return true;
    } catch (error: any) {
      console.error('Failed to update config files:', error);
      throw new Error(error.message || 'Failed to update configuration files');
    }
  };
